...
GEN = "python -m rlcompopt.cl.generate --config-path conf/rl_online --config-name generate_autophase "
train = "python -m rlcompopt.train --config-path cl/conf/rl_online --config-name train_autophase "
TRAIN = {
    "autophase": train,
}
...
train_config = {"optim.lr": "5e-5"}
exp.append((gen_config.copy(), train_config.copy()))

train_config = {"optim.lr": "7e-5", "model.entropy_factor": "0.0005"}
exp.append((gen_config.copy(), train_config.copy()))
...
def submit_all(dry_run=True):
    ...
    submit("autophase", dry_run=dry_run)
...
def parse_args():
    ...
    parser.add_argument(
        "--log_dir", default="outputs_autophase_ppo/", type=str,
